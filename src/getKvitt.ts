...
export const getBalance = (database: Database): Record<string, number | undefined> => {
...
export const getKvitt = (
  ...
  while (Object.values(currentBalance).some((x) => x !== 0) && attemptsLeft > 0) {
    attemptsLeft--
    ...
    const extremes = Object.entries(currentBalance).reduce(
      (acc, tuple) => {
        ...
        const max = !acc.max || value > acc.max.value ? { person: name, value } : acc.max
        const min = !acc.min || value < acc.min.value ? { person: name, value } : acc.min
        return { min, max }
      },
    ...
    if (!extremes.min || !extremes.max || extremes.max.person === extremes.min.person) {
      break
    }
    ...
    paymentsForBalance.push({
      ...
      payment: Math.min(Math.abs(extremes.min.value), Math.abs(extremes.max.value)),
    })
